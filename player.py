...
class Player(Character):
  ...
  def quit(self):
    ...
    self.health = 0
  def help(self):
    #print ("q=Quit e=Explore r=Rest a=Attack f=Flee s=Status h=Help")
    print (self.Commands.keys())
  def status(self): print ("%s's strenght: %d/%d" % (self.name, self.health, self.health_max));
  def tired(self):
  ...
  def attack(self):
    if self.state != 'fight': print ("%s swats the air, without notable results." % self.name); self.tired()
      ...
      if self.do_damage(self.enemy):
        ...
        print("Your Wealth is %d now... " %self.wealth)
        self.enemy = None
        self.state = 'normal'
